...
/// AST-driven interprocedural data-flow analysis for Java files, mirroring the
/// C/C++ `CAnalyzer`. It reuses the shared `CStmt`/`CExpr` representation by
/// parsing each method body with `CParser`, then computes:
...
/// It is a structural copy of `CAnalyzer` but seeded with the Java source/write
/// sets from `JavaPlatform.swift` instead of the C ones.
public struct JAnalyzer {

    private let astFns: [String: CFunctionDef]

    public init(source: String, methods: [JMethodDef]) {
    ...
    /// sink/taint detection over Java method bodies.
    ...
    private func functionReturnsTaint(_ fn: CFunctionDef, taintReturning: Set<String>) -> Bool {
        ...
        let seedFns = javaSourceAPIs.union(taintReturning)
...
switch e {
        ...
        case .call(let callee, let args, _):
            if case .identifier(let cname, _) = callee, javaWriteThroughSinks.contains(cname) {
    ...
    /// Computes which parameters of each function are tainted at some call site
    /// in the file. A parameter is seeded as tainted only when the function is
    /// actually called with a tainted argument — this is the cross-file
    /// parameter provenance analysis that prevents false positives for functions
    /// whose parameters are only ever passed sanitized values.
    ...
    private func walkTaintAndRecord(
        ...
        case .expr(let e):
            ...
            updateTaintFromExpr(e, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .declaration(let d):
            if case .variable(_, _, let initExpr) = d.kind {
                if let expr = initExpr { updateTaintFromExpr(expr, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning) }
            }
        case .ifStmt(_, let t, let e, _):
    ...
    private func updateTaintFromExpr(_ e: CExpr, tainted: inout Set<String>, taintedLocals: inout Set<String>, seededParams: Set<String>, seedFns: Set<String>, taintReturning: Set<String>) {
switch e {
            case .assign(_, let lhs, let rhs, _):
                if simpleIdentifier(lhs) != nil {
                if isSanitizedCall(rhs, seedFns: seedFns, taintReturning: taintReturning) {
                    // clean result
                } else if exprTainted(rhs, tainted: tainted, seedFns: seedFns, taintReturning: taintReturning) != nil {
                    if let n = simpleIdentifier(lhs) { tainted.insert(n); taintedLocals.insert(n) }
                }
            }
            updateTaintFromExpr(lhs, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(rhs, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .call(let callee, let args, _):
            for a in args { updateTaintFromExpr(a, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning) }
            updateTaintFromExpr(callee, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .member(let b, _, _, _):
            updateTaintFromExpr(b, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .index(let b, let idx, _):
            updateTaintFromExpr(b, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(idx, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .unary(_, let o, _):
            updateTaintFromExpr(o, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .binary(_, let l, let r, _):
            updateTaintFromExpr(l, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(r, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .comma(let l, let r, _):
            updateTaintFromExpr(l, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(r, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .ternary(let c, let t, let f, _):
            updateTaintFromExpr(c, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(t, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
            updateTaintFromExpr(f, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .cast(let x, _), .paren(let x, _):
            updateTaintFromExpr(x, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning)
        case .arrayInit(let arr, _):
            for a in arr { updateTaintFromExpr(a, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning) }
        case .newExpr(_, let args, _):
            for a in args { updateTaintFromExpr(a, tainted: &tainted, taintedLocals: &taintedLocals, seededParams: seededParams, seedFns: seedFns, taintReturning: taintReturning) }
        default: break
        }
    ...
    private func recordCallSites(_ e: CExpr, tainted: Set<String>, seedFns: Set<String>, callSites: inout [(name: String, indices: Set<Int>)]) {
        ...
        case .comma(let l, let r, _):
            ...
            recordCallSites(r, tainted: tainted, seedFns: seedFns, callSites: &callSites)
        case .ternary(let c, let t, let f, _):
            recordCallSites(c, tainted: tainted, seedFns: seedFns, callSites: &callSites)
            recordCallSites(t, tainted: tainted, seedFns: seedFns, callSites: &callSites)
            recordCallSites(f, tainted: tainted, seedFns: seedFns, callSites: &callSites)
        case .cast(let x, _), .paren(let x, _):
            recordCallSites(x, tainted: tainted, seedFns: seedFns, callSites: &callSites)
        case .arrayInit(let arr, _):
            for a in arr { recordCallSites(a, tainted: tainted, seedFns: seedFns, callSites: &callSites) }
        case .newExpr(_, let args, _):
            for a in args { recordCallSites(a, tainted: tainted, seedFns: seedFns, callSites: &callSites) }
        default: break
        }
    ...
    private func isSanitizedCall(_ e: CExpr, seedFns: Set<String>, taintReturning: Set<String>) -> Bool {
        return false
    }
    ...
    private func exprTainted(_ e: CExpr, tainted: Set<String>, seedFns: Set<String>, taintReturning: Set<String>) -> String? {
        ...
        case .call(let callee, let args, _):
            ...
            if let q = callQualifiedName(callee), seedFns.contains(q) { return q }
            if let m = callName(callee), seedFns.contains(m) { return m }
            if let t = exprTainted(callee, tainted: tainted, seedFns: seedFns, taintReturning: taintReturning) { return t }
            if case .member(let base, _, _, _) = callee, let t = exprTainted(base, tainted: tainted, seedFns: seedFns, taintReturning: taintReturning) { return t }
            if isSanitizedCall(e, seedFns: seedFns, taintReturning: taintReturning) { return nil }
    ...
    private func callName(_ callee: CExpr) -> String? {
        ...
        case .call(let innerCallee, _, _): return callName(innerCallee)
        default: return nil
        }
    }

    private func callQualifiedName(_ callee: CExpr) -> String? {
        switch callee {
        case .identifier(let n, _): return n
        case .member(let base, let m, _, _):
            if let bn = callQualifiedName(base) { return bn + "." + m }
            return m
        case .call(let innerCallee, _, _): return callQualifiedName(innerCallee)
        default: return nil
        }
    }
}
